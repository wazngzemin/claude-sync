...
1. 依据[《可见即可说框架需求文档》](https://bytedance.larkoffice.com/wiki/AxYjwUI8wiFCXWkdNsDcRae0nCf)，把旧版“有限词条直接回 Trigger”改为完整链：当前顶层页面解析 → 指令/元素 ID → 在线/离线 ASR → 指令匹配 → 注册方消歧 → 模拟点击 → 卡片正常按钮回调。
2. 明确可见即可说没有业务推理能力，也不是即时交互卡内部的一段逻辑；页面方、字节语音、VUI、Trigger 各自有不同责任。
3. 区分即时任务、即时任务卡、即时交互卡、任务中心和 AI Bar 任务岛，不再用“即时任务”泛指所有弹卡场景。
4. 将“UI 模拟点击成功”和“车辆动作真实成功”拆成两个状态；前者不能作为最终成功提示。
5. 补充郝晓伟 9 月 1 日最新版任务中心 PRD：系统预设任务已纳入展示范围，但任务单元仍待补充、一期操作仅写删除，列表推送/拉取也不能证明 Trigger 已取得任务有效状态。

> 注意：本版确认的是文档能力定义和应有产品合同，不代表天气卡已经完成可见即可说接入，也不代表 Task Service、任务中心、Trigger 之间的正式接口已落地。
...
| TTS | 把文字播成语音 |
| 可见即可说 | 一套车端“用语音操作当前最上层可见界面”的框架：业务解析页面元素并生成指令与元素 ID，字节语音完成 ASR 和指令匹配，注册方消歧后由车企/业务模拟点击对应控件。它不理解业务条件，也不直接返回车控成功 |
| 即时任务 | 能在当前轮次或较短时间内完成的业务任务类型；它是任务对象，不是一个 UI 模块 |
| 即时任务卡 | 面向即时任务创建、触发或关闭结果的瞬时、非生成式反馈卡；是否直接复用即时交互卡容器，文档命名尚需专项统一 |
| 即时交互卡/临时交互卡 | VUI 的通用临时结果容器，可承载 Query 或主动服务的文本、图片、控件与按钮；范围大于“即时任务卡” |
| AI Bar 任务岛 | AI Bar 中持续展示有效长时/长程任务的快捷入口；不是即时交互卡，也不是任务执行器 |
...
5. 如果用户点击“确认”，卡片按正常按钮事件回调；如果用户说“好的”，并不是语音直接回 Trigger，而是进入可见即可说链路。
6. 可见即可说用当前最上层页面的“按钮文案—元素 ID”映射匹配语音，由页面注册方消歧后模拟点击“确认”按钮。
7. 即时交互卡把手点或模拟点击统一变成同一类卡片按钮事件，并带回本次运行事件，交还原天气业务。
8. 端侧在真正车控前再读一次任务有效状态、天气、车速和当前驾驶模式。
9. 条件仍成立才请求赛力斯车控；最后读取真实驾驶模式，确认车辆真的切换成功。

这九步里，任何一步没有负责人、输入、输出或失败规则，需求就不完整。9 月 1 日评审卡住的根本原因，就是大家看到了四个 Case，却没有看到背后的公共框架；此前 PRD 还把“语音识别成功”“按钮被模拟点击”和“车辆动作成功”写成了一个结果，本版已拆开。
...
3. **即时交互卡负责承载“问什么”和控件交互；可见即可说负责把语音映射成当前页面控件的模拟点击。** 两者都不判断天气、SOC 或充电设备，也不自行决定车控。
4. **云端和端侧的区别不在卡片显示在哪里。** 两种卡都显示在车机；区别在判断、语音理解、上下文、点击回流和执行编排分别由谁负责。
5. **点击确认不等于车辆已经完成。** 用户授权后仍要端侧读取最新状态、执行车控并以真实状态回读作为最终成功口径。

再补一个本版必须统一的概念：**即时任务是业务对象；即时任务卡是该类任务的瞬时反馈卡；即时交互卡是更通用的 VUI 容器；任务中心是长期管理与回看入口；AI Bar 任务岛是长时/长程任务的持续入口。** 五者不能互相替代。
...
flowchart LR
    ...
    H -- 是 --> J{是否需要用户确认}
    I -- 是 --> J
    J -- 是 --> K[即时交互卡<br/>展示 + 可选TTS + 手点/模拟点击控件事件]
    J -- 否 --> L[端侧执行前复核]
    K --> M{用户是否有效确认}
...
| 端侧卡合同 | 确认拉卡、展示 ACK、按钮元素 ID、手点/模拟点击同路回调、关闭、超时和最终更新 |
| 可见即可说接入合同 | 确认页面解析/映射、在线离线 ASR、匹配、消歧、模拟点击、历史标记和失效责任 |
...
| 徐洋｜VUI/即时交互卡 | 四类卡片调用链；卡片准入、生命周期、TTS、按钮元素 ID、手点/模拟点击控件事件、云端上下文 | 天气端侧卡合同、充电云端卡合同、能力缺口 |
| 左晨/字节语音 + 赛力斯页面方 | 可见即可说的页面解析、指令映射、在线/离线 ASR、匹配、消歧、模拟点击和历史标记 | 完整责任时序、天气卡最小接入和验收边界 |
...
| 01:36:10–01:40:13 | 天气卡的可见即可说找谁做 | 大家混淆了“天气业务接入”“卡片控件能力”和“语音框架” | 天气需要本地确认；方向上使用可见即可说 | 谁解析/注册页面、谁做 ASR/匹配和消歧、谁模拟点击、卡片怎样同路回调、是否已完成 |
...
| 卡片语音路径 | 可见即可说匹配当前顶层页面元素并模拟点击；无业务推理 | Planner 结合卡片上下文做泛化语义理解 |
...
| 即时任务 | 能在当前轮次或较短时间内完成的任务类型，是业务对象 | 由原业务/Task Service 创建、分类、执行和维护结果 | 不是卡片、任务中心或 Trigger |
| 即时任务卡 | 即时任务创建、触发、关闭时出现的瞬时、非生成式反馈卡 | 承接即时任务结果反馈；实际渲染容器和接口需专项统一 | 不保存长期任务，不运行任务逻辑 |
| 即时交互卡 | VUI 的通用临时结果容器；可承载 Query、主动服务、系统通知等结果 | 展示卡片、承接控件操作、管理生命周期并把事件回给原业务 | 不判断业务条件，不把按钮点击直接当车控成功 |
| 可见即可说 | 用语音操作当前最上层可见页面的一套跨模块框架 | 页面解析/注册、ASR、指令匹配、消歧、模拟点击共同完成 | 没有业务推理能力；不判断天气，也不直接输出车辆动作成功 |
| AI Bar 任务岛 | AI Bar 中持续展示有效长时/长程任务的快捷入口 | 聚合并展示仍在运行的长任务 | 不是即时卡，也不负责执行任务 |
...
```text
即时任务（业务对象）
  └─ 创建/触发/关闭时，可用“即时任务卡”做瞬时反馈
       └─ 该卡可能由通用“即时交互卡”容器承载，正式命名和接口待统一
...
可见即可说
  └─ 不是任何一种卡，而是让用户用语音操作“当前可见卡片/页面控件”的能力
```

【资料冲突】AIDV Task Service 技术方案在通知链里混用了“即时任务卡、即时卡、即时交互卡”；FeatureList 则把“即时任务卡”单独定义为面向即时任务的非生成式瞬时卡。因此本 PRD 只确认上面的产品关系，不宣布三者完全同义；正式组件名、调用接口和 Owner 必须由 Task Service 与 VUI 专项统一。

Task Service 技术方案当前给出的展示组合是：

| Task Service 任务类型 | 瞬时卡反馈 | AI Bar 任务岛 |
|---|---|---|
| 单次定时/单次条件 | 使用 | 不使用 |
| 循环定时/通用长时 | 使用 | 使用 |
| 长程/生态任务 | 不以瞬时卡为主 | 使用 |

这张表描述的是 **Task Service 创建的任务如何反馈/持续露出**，不能直接套到天气等系统预设任务。天气是否经过 Task Service、是否进入任务岛，都需要单独证据。
...
| 即时交互卡/VUI | 车机 VUI | 卡片内容、按钮、TTS 策略、路由和有效期 | 能否展示、当前生命周期、控件是否仍有效 | 卡片显示/生命周期；手点或模拟点击形成的同一类控件事件 | 失败、关闭、超时均明确回业务；不把 UI 点击成功报成车控成功 |
| 页面注册方（车企/卡片业务） | 车端 UI/业务模块 | 当前最上层页面的可交互元素 | 哪些元素可说、文案如何去重、页面变化后哪些映射失效 | 指令/同义表达/元素 ID 映射；多结果消歧决定 | 页面隐藏、刷新或元素失效时废弃旧映射；无法消歧则不点击 |
| 字节语音 ASR | 车端+在线语音链 | 当前页面动态热词、用户语音 | 免唤醒或唤醒后走何种识别；在线/离线结果如何仲裁 | ASR 文本 | 识别失败不产生点击；具体提示策略待确认 |
| 字节语音指令匹配 | 车端语音模块 | ASR 文本、当前页面指令集 | 精确/模糊/本地匹配；是否出现多个候选 | 匹配到的指令集和元素 ID | 无结果不操作；多结果返回注册方消歧 |
| UI 模拟点击执行方（车企/业务） | 车端 UI/业务模块 | 消歧后的元素 ID | 元素是否仍属于当前最上层页面且可操作 | 模拟点击、点击动画、UI 操作结果；随后进入卡片正常点击回调 | 元素失效或操作失败不产生业务授权，并按确认策略反馈 |
...
1. 【文档事实】郝晓伟 9 月 1 日版[《赛力斯任务中心 PRD》](https://bytedance.larkoffice.com/wiki/LUKKwMTNaiAd53kdmpkc5oILn5e)已把“系统预设任务”纳入任务中心统一展示和管理范围；因此不能再写成“任务中心是否包含系统预设任务完全未知”。
2. 【文档事实】同一最新版里，“系统预设任务”的具体任务单元展示仍标为“待补充”；它证明了范围方向，不证明四条任务的页面、协议或运行链已经接通。
3. 【资料冲突】最新版一期任务操作只写“删除”，没有写启用/停用；较早通用任务中心方案及本项目天气需求却出现开关。因此“天气任务有效开关由谁提供、是否属于任务中心一期”仍必须请郝晓伟拍板。
4. 【文档事实】最新版写明：任务更新时云端向车机推送；车辆上电或账号登录时，车机主动拉取任务列表。**这描述的是任务中心列表更新/恢复，不等于 Trigger 已收到可执行的任务有效状态。**
5. 【文档事实】通用设计中，任务中心负责展示、排序、组件和用户操作事件转发；接入业务维护真实任务状态、执行并回传结果。用户点击只表达意图，不能把页面显示当作 Trigger 的状态真源。
6. 【会议原话｜非结论】郝晓伟称任务中心可以理解为 HMI 展示端；系统预设条件任务可能在 Trigger 配置并通过“其他路径”创建，而不是都由 Director 创建。
7. 【文档事实】通用任务协议出现过 `task_id`、用户事件等概念，但没有证据证明四条系统预设任务已经通过 `taskId + enabled` 直接推送给 Trigger。
...
【待确认】“方向为端侧”不等于端侧离线即时卡、页面元素解析/映射、ASR/匹配、模拟点击、全部信号和状态回读已经完成。任务中心 PRD 的无网限制与 7 月 28 日端侧卡目标存在冲突，本次必须通过接口和实车证据确认。
...
flowchart TD
    ...
    V -- 是 --> J[按策略播TTS]
    V -- 否 --> J2[只显示卡片并开放手点]
    J --> P1[车企/卡片业务解析当前最上层页面<br/>生成按钮文案、同义表达与元素ID映射]
    P1 --> K{用户怎样操作?}
    J2 --> K
    K -- 手动点击 --> C1[即时交互卡产生正常按钮事件]
    K -- 说好的/确认 --> S1[字节语音ASR<br/>免唤醒纯离线；唤醒后在线/离线并行]
    S1 --> S2[字节语音做精确/模糊/本地匹配<br/>输出候选指令与元素ID]
    S2 --> S3{候选是否唯一且仍有效?}
    S3 -- 否/无结果 --> X
    S3 -- 多个 --> S4[页面注册方结合位置/上下文消歧]
    S3 -- 唯一 --> S5[页面注册方检查元素仍在当前最上层页面]
    S4 --> S5
    S5 --> S6[车企/卡片业务模拟点击目标按钮<br/>展示点击动画]
    S6 --> C1
    C1 --> L[卡片带同一运行事件回调按钮语义<br/>手点与模拟点击走同一业务入口]
    K -- 拒绝/关闭/超时/打断 --> X
    L --> Y[标记本次卡片已消费<br/>页面变化后旧元素映射失效]
...
| 9 | TTS/VUI | 卡片真正展示状态、TTS 策略、语音禁用状态 | 是否允许播报 | 播报或静音展示 | 禁语音时天气只保留卡片和点击；不因为 TTS 失败自动车控 | 天气降级方向【会议确认】；接口【待确认】 |
| 10 | 页面注册方：车企/卡片业务 | 当前最上层页面的按钮、文本、位置和状态 | 哪些元素可交互；重名怎样加入位置/上下文；页面是否变化 | 动态指令/同义表达/元素 ID 映射，并作为 ASR 热词 | 页面隐藏、刷新或元素变化后旧映射失效；解析失败保留手点 | 可见即可说框架【文档事实】；天气接入【待确认】 |
| 11 | 字节语音 ASR | 用户语音、当前动态热词 | 免唤醒纯离线；唤醒后在线/离线并行，离线有结果优先执行 | ASR 文本 | 无结果不产生 UI 操作 | 【文档事实】 |
| 12 | 字节语音指令匹配 | ASR 文本、当前指令集 | 精确、模糊或本地匹配；是否多个候选 | 候选指令集和元素 ID | 无匹配不操作；多个匹配交还注册方消歧 | 【文档事实】 |
| 13 | 页面注册方 + UI 执行方 | 候选元素 ID、当前页面 | 是否仍是最上层有效元素；多候选如何消歧 | 对目标按钮模拟点击、点击动画、UI 操作结果 | 无法消歧/元素失效/模拟点击失败均不形成业务授权 | 【文档事实】；具体 Owner【待确认】 |
| 14 | 即时交互卡 | 用户手点，或可见即可说形成的模拟点击 | 控件是否仍有效、卡片是否已消费 | 同一类按钮事件，并保留输入来源、运行事件关联 | 旧卡、重复点击、过期事件不回有效授权 | 业务合同【产品方案】 |
| 15 | 原天气业务/Trigger | 卡片按钮事件、关闭/超时和本次事件关联 | 是否表达有效确认；是否属于当前运行事件 | 进入复核或结束 | 拒绝、关闭、超时全部不执行 | 【产品方案】 |
| 16 | 端侧执行入口 | 最新任务状态和车辆状态 | 场景仍成立、目标未完成、授权有效、无冲突、未重复 | 允许或拒绝执行 | 变化或过期时终止并返回原因 | 【产品方案】 |
| 17 | 赛力斯车控 | 通过复核的具体动作 | 整车能力和安全门禁 | 请求接收/拒绝/错误 | 超时或拒绝不得显示成功 | 车控在端侧【会上宣讲方案】；正式合同【待确认】 |
| 18 | 状态域 + 原天气业务 | 最新真实模式/灯状态 | 是否达到本次目标 | 成功、失败或未知；更新卡片/日志/冷却 | ACK 成功但状态没变仍判失败/未知 | 状态回读要求【产品方案】 |
...
7. 接收即时交互卡的按钮业务事件；该事件可能来自用户手点，也可能来自可见即可说完成的 UI 模拟点击。Trigger 不直接接 ASR 文本或“可见即可说标准结果”。
...
3. 在真正展示后，按策略协同 TTS；同时向页面解析/注册机制暴露可操作按钮的显示文案、稳定元素 ID 和当前状态。
4. 接受用户手点或外部可见即可说模块触发的模拟点击，并走同一个正常按钮回调；卡片本身不负责 ASR、指令匹配或语义推理。
...
- 决定用户确认后一定执行；
- 直接持有一条不带业务复核的裸车控指令；
- 在卡片已消失后继续让旧页面映射或旧元素 ID 生效。

可见即可说的责任不应塞进“卡片负责”中：页面/卡片业务负责解析可交互元素并模拟点击，字节语音负责 ASR 与指令匹配，注册方负责多候选消歧；即时交互卡只需保证模拟点击与手点进入同一按钮事件合同。
...
| 卡片展示 | 展示问题和确认/拒绝，并暴露可交互元素 | 正常时播 TTS；页面侧动态生成指令与元素 ID 映射 | 开始等待本次用户选择 |
| 语音禁用 | 天气保留卡片 | 不播 TTS、不接语音，仅点击 | 点击确认仍需复核 |
| 用户确认 | 显示处理中或等待业务更新 | 手点或模拟点击进入同一按钮回调；页面变化后旧映射失效 | 立即重新读取最新状态 |
| 用户拒绝 | 关闭或显示已取消 | 同上 | 不执行，记录并按策略冷却 |
| 用户关闭/超时 | 卡片结束 | 旧元素映射和元素 ID 必须失效 | 视为未授权，不执行 |
| 被用户 Query/更高优交互打断 | 隐藏或结束，规则待 VUI 确认 | 隐藏后不得继续用旧元素 ID 模拟点击 | 若没有有效确认则不执行 |
...
| 汪帅等端侧研发 | 当前天气链已经做到哪一步；Trigger 拉卡、卡片按钮回调、可见即可说模拟点击、车控、状态回读哪些已接 | 实现证据和联调计划 |
| 徐洋/VUI | 端侧自闭环卡拉起、排队、展示、关闭、超时如何回调；卡片如何暴露按钮文案/元素 ID，模拟点击是否走正常按钮事件 | 端侧卡业务时序和正式对接说明 |
| 左晨/字节语音 + 赛力斯页面注册方 | 谁解析最上层页面、生成动态指令与元素 ID；ASR/匹配、多候选消歧、模拟点击分别是谁；页面变化如何失效 | 可见即可说完整责任图、现有接口和天气卡接入说明 |
| VLM/端状态 | 天气/路面正式字段、枚举、周期、时间戳、质量和准确率 | 数据字典与测试样本 |
| 赛力斯产品/车控 | 三条场景的唯一规则、动作、安全门禁和恢复策略 | 生效需求表、动作/状态能力清单 |

### 7.5.9 端侧链路备答话术

> “天气是这次端侧框架的样例。用户开启任务后，Trigger 必须先拿到明确有效的任务状态，再读取本地 VLM 天气路况和端状态。命中规则、通过防抖去重后，Trigger 产生一次运行事件并请求即时交互卡。用户手点时，卡片直接产生按钮事件；用户说‘好的’时，语音不是直接回 Trigger，而是由可见即可说完成页面元素解析、ASR、指令匹配、消歧和 UI 模拟点击，再由卡片走同一按钮回调。Trigger 收到的始终是本次卡片的业务事件，随后再读一次最新任务和车况，通过才请求赛力斯车控，最后以真实模式或灯状态收口。今天要分别拍板卡片合同和可见即可说接入，不能把 UI 模拟点击成功当成车辆执行成功。”
...
## 7.7 即时任务、即时交互卡与可见即可说：三个概念、两条语音链

### 7.7.1 先把“任务”和“卡”彻底分开

| 名称 | 它到底是什么 | 出现在哪里 | 与天气任务的关系 |
|---|---|---|---|
| 即时任务 | 可以在当前轮次或较短时间内完成的**业务任务类型** | Task Service/原业务的任务分类 | 天气保护是长期存在、等条件命中的系统预设任务，不应因为命中后弹一张卡就改叫“即时任务” |
| 即时任务卡 | 即时任务在创建、触发或关闭时使用的**瞬时、非生成式反馈卡** | Task Service 通知链 | 没有证据证明天气一定走 Task Service 的即时任务卡接口 |
| 即时交互卡/临时交互卡 | VUI 的**通用临时结果容器**，可承载 Query、主动服务、系统通知等结果 | 车机 VUI | 天气命中后使用第④类端侧自闭环服务通知卡，是即时交互卡的一种业务用法 |
| 任务中心 | 跨任务的持续管理、操作与回看页面 | 车机 HMI | 用于展示/管理天气等系统预设任务，不负责判断天气或弹卡 |
| AI Bar 任务岛 | 长时/长程任务仍在运行时的持续快捷入口 | AI Bar | 不是天气命中后的确认卡；天气是否需要任务岛没有资料结论 |

```text
“即时任务”回答：这是什么类型的任务？
“即时任务卡”回答：即时任务在某个节点如何瞬时反馈？
“即时交互卡”回答：车机用什么通用容器展示一次结果和按钮？
“可见即可说”回答：用户怎样用语音操作当前可见的按钮？
```

【资料冲突】AIDV Task Service 方案在同一通知链中混用了“即时任务卡、即时卡、即时交互卡”，而 FeatureList 把“即时任务卡”单独定义为面向即时任务的非生成式卡。因此在正式 IDL 统一前，本 PRD 不把二者宣布为同一个组件。
...
### 7.7.3 可见即可说到底是什么：不是“几条确认词”，而是一套 UI 语音操作框架

[《可见即可说框架需求文档》](https://bytedance.larkoffice.com/wiki/AxYjwUI8wiFCXWkdNsDcRae0nCf)描述的是下面这条完整链，而不是“卡片把语音直接返回 Trigger”：

```mermaid
sequenceDiagram
    actor U as 用户
    participant UI as 当前最上层页面/卡片
    participant REG as 页面注册方（车企/业务）
    participant ASR as 字节语音 ASR
    participant MAT as 字节语音指令匹配
    participant CARD as 即时交互卡按钮事件
    participant BIZ as 原业务/天气 Trigger

    UI->>REG: 当前可交互元素、显示文案、位置、状态
    REG->>REG: 生成指令/同义表达/元素ID映射<br/>重名时加入位置或上下文
    REG-->>ASR: 当前页面动态热词/指令集
    U->>ASR: 说“好的/确认/暂时不用”等
    ASR->>MAT: ASR文本
    MAT->>MAT: 精确/模糊/本地匹配
    MAT-->>REG: 候选指令集 + 元素ID
    alt 多个候选
        REG->>REG: 结合页面位置/上下文消歧
    end
    REG->>REG: 再确认元素仍在当前最上层页面
    REG->>UI: 模拟点击目标元素并展示点击动画
    UI->>CARD: 产生与手点相同的正常按钮事件
    CARD-->>BIZ: 按钮业务语义 + 运行事件关联 + 输入来源
    BIZ->>BIZ: 重新检查任务与车辆状态，再决定是否车控
```

#### 六步分别是谁负责

| 步骤 | 谁负责 | 输入 | 判断/动作 | 输出给下一步 | 不成功时 |
|---:|---|---|---|---|---|
| 1. 解析页面 | 车企或具体页面/卡片业务 | 当前最上层页面的按钮、输入框、列表项、菜单等 | 只解析当前顶层可见页面；过滤不适合作为语音指令的符号/标点 | 可交互元素清单 | 解析不到则该元素不可说，但仍可手点 |
| 2. 生成映射 | 车企或业务注册方 | 显示文案、位置、控件状态 | 生成热词、泛化表达和元素 ID；重名元素增加位置/上下文；页面变化时动态更新 | 指令 ↔ 元素 ID 映射 | 旧页面/旧元素映射必须失效 |
| 3. 语音识别 | 字节语音 ASR | 用户语音、动态热词 | 免唤醒走纯离线；唤醒后在线/离线并行，有离线结果则优先执行，否则等在线 | ASR 文本 | 无可靠结果，不操作 UI |
| 4. 指令匹配 | 字节语音匹配模块 | ASR 文本、当前页面指令集 | 做精确、模糊和本地匹配 | 匹配指令及元素 ID；可能一个或多个 | 无匹配不操作；多个交注册方 |
| 5. 消歧与模拟点击 | 页面注册方/车企 UI | 候选元素 ID、当前页面 | 多候选消歧；验证元素仍有效；模拟点击并显示点击动画 | UI 操作结果，并触发控件正常事件 | 无法消歧、元素失效或点击失败，不形成有效业务授权 |
| 6. 业务回调 | 即时交互卡 + 原业务 | 手点或模拟点击产生的控件事件 | 校验卡片/运行事件未过期、未消费 | 同一次天气事件的确认/拒绝按钮语义 | 关闭、超时、旧事件、重复事件均不执行 |

#### 原文明确的匹配边界

- 支持“点击、点一下、帮我选、选择 + 当前指令”等通用前缀，也允许部分语气后缀；这属于指令匹配，不是业务推理。
- 当字符串长度不小于 6 个字符时，原文给出的模糊规则是至少连续命中 4 个字符。
- 单页面指令规模上限当前按 500 词描述；天气卡只有少量按钮，但仍应把这项纳入框架验收。
- 匹配到多个元素时，匹配模块把多个候选与元素 ID 交回注册方消歧，不能随便选第一个。
- 可见即可说操作需要进入对话历史，并标记为“模拟点击事件”，避免下一轮模型误以为用户说的是一个普通 Query；具体由谁写入、写入何种协议仍待专项确认。
- 原文给出的语音意图优先级为：P0 用户自定义任务 ＞ P1 可见即可说 ＞ P2 FAQ ＞ P3 端侧 LLM ＞ P4 云端 LLM。**这是语音意图识别优先级，不等于即时卡与所有主动服务的全局调度优先级。**
- 可见即可说没有业务推理能力。它能把“好的”映射成当前“确认”按钮，但不能判断雨是否还在、车速是否安全、是否应该切换模式。

【资料冲突】原文对失败反馈同时出现“操作失败反馈 TTS”和“失败反馈文本不播报”的不同表述。本 PRD 只要求失败不得误报成功，具体是否播报、播什么由语音/VUI 专项拍板。

### 7.7.4 天气 Trigger 和可见即可说到底怎样接起来

```text
Trigger 命中天气规则并拉起端侧自闭环卡
→ 卡片真正展示“确认/暂不”按钮
→ 页面注册方让按钮进入当前可见元素映射
→ 用户手点：卡片直接产生按钮事件
→ 用户说“好的”：ASR/匹配/消歧后模拟点击同一个“确认”按钮
→ 两条路都由卡片回同一个天气业务入口，并关联同一次运行事件
→ Trigger 执行前重新检查任务开关、天气、车速、模式
→ 赛力斯车控
→ 读取真实车辆状态，更新最终结果
```
...
| 成功口径 | 说明 | 能不能对用户说“车辆已完成” |
|---|---|---|
| UI 模拟点击成功 | 元素仍有效，系统成功触发了按钮事件 | **不能**。只说明用户选择被送达业务 |
| 业务/车辆执行成功 | Trigger 复核通过、车控完成且真实车辆状态达到目标 | **可以**。这是最终成功口径 |

因此 Trigger 研发需要的不是一条“ASR → CONFIRM”接口，而是：即时交互卡在手点或模拟点击后，都把同一按钮的业务语义、当前卡片/运行事件关联、输入来源和生命周期状态回给天气业务。正式回调接口仍由 VUI 与端侧研发确认。

### 7.7.5 “卡片有泛化能力”说的是云端增强链，不是本地可见即可说天然会推理

| 对比项 | 本地可见即可说 | 云端卡片泛化/增强可见即可说 |
|---|---|---|
| 核心能力 | 当前页面指令匹配后模拟点击 | Planner/DT 结合卡片上下文理解开放表达 |
| 是否需要模型推理 | 不需要，也不具备业务推理 | 需要云端 Planner/动态工具能力 |
| 用户表达 | 更适合“确认、暂不、打开后雾灯”等与可见项接近的说法 | 可处理“先别开”“这附近真的有吗”“换一个充电站”等上下文表达 |
| 输入 | 当前最上层页面的指令/元素 ID | 卡片标题、正文、按钮、上下文和用户 Query |
| 输出 | 模拟点击某个当前元素 | `select_button` 类选择、其他 Query 或无法判断；正式接口待确认 |
| 弱网 | 本地链有离线识别方向 | 依赖云端上下行和 Planner/DT |
| 天气/充电关系 | 天气第④类端侧自闭环卡采用本地链 | 充电若采用云端卡，语音按最终卡型进入 Planner/动态选择链 |

云端“泛化”不是卡片自己装了模型，而是卡片存在期间把标题、内容、按钮和业务上下文提供给 Planner，使 Planner 临时拥有“当前卡片可选择项”。技术资料曾使用 `real_time_card`、动态选择工具、`select_button` 等表达该设计；这能证明设计方向，不证明充电业务已经接入。

### 7.7.6 Trigger/云端业务给卡片什么
...
| 按钮显示文案、稳定元素 ID 与业务含义 | 让手点、模拟点击和业务回调指向同一控件、同一语义 | `确认`/`暂不` + 各自元素 ID + 业务语义 | `打开`/`暂不` + 业务语义 |
| 本地可见即可说别名（可选） | 补充界面文案自然泛化无法覆盖的业务说法；不是让产品穷举所有口语 | 可提供“好的/可以/不用”等样例，最终由页面注册与匹配机制确定 | 云端 Planner 链不要求穷举固定词 |
...
| 卡片准入/调度信息 | 供 VUI 与其他卡片、页面和交互做排队/替换/取消 | 具体等级待 VUI/架构确认 | 同左 |
| 当前业务状态 | 支持显示等待、处理中或结果 | 等待确认 | 等待确认/正在开盖 |

这里有一个不能在评审时讲错的边界：**当前资料只能证明“天气方向使用可见即可说”，不能证明现有所有即时交互卡已经自动接入该框架。** 天气业务负责定义按钮业务语义和必要样例；页面元素由谁解析/注册、卡片如何暴露稳定元素 ID、页面变化如何让旧映射失效、模拟点击是否走正常回调，需要 VUI、左晨语音、赛力斯页面方和端侧研发共同确认。未确认前，不能写成“卡片一展示就天然支持语音”。

另外，原可见即可说文档的 P0–P4 是**语音意图识别优先级**，不能拿来证明天气卡一定低于用户 Query，也不能替代 VUI 的全局卡片调度合同。
...
| 生命周期结果 | 隐藏、恢复、过期、被替换、被消费 | 让旧页面元素映射失效，或销毁 Planner 动态上下文，避免旧授权 |
...
stateDiagram-v2
    ...
    Hidden --> Revalidating: 旧页面元素映射不可继续使用/云端动态选择暂停
    Revalidating --> Visible: 重新解析当前页面，且任务、事件、卡片和推理仍有效
...
“其他 Query”不默认等于卡片已经被消费；它可能只是打断或转场。卡片进入 Hidden 后已不再是当前最上层页面，旧元素映射不得继续用于模拟点击；恢复时需要重新解析当前页面，并重新校验任务、运行事件、卡片本身和推理结果是否仍有效。云端链还要同步暂停或销毁动态选择上下文。

### 7.7.10 卡片、可见即可说与 Trigger 的边界验收

| 用例 | 正确结果 |
|---|---|
| Trigger 条件未满足 | 不请求卡片；卡片不自行订阅天气或 SOC |
| 卡片排队但尚未展示 | 不播 TTS、不开始用户授权；不可被当成当前可见页面操作 |
| 卡片展示失败 | 需要授权的任务停止，不能静默车控 |
| 用户点击确认 | 卡片返回本次选择；原业务复核，不由卡片直接车控 |
| 用户说“好的”，当前顶层就是天气卡 | ASR/匹配/消歧后模拟点击当前确认按钮，再由卡片正常回调原业务 |
| 页面有两个同名“确认”按钮 | 返回多个元素 ID 给页面注册方消歧；无法消歧不点击 |
| 天气卡已隐藏/过期后说“好的” | 旧页面映射不可用；云端上下文如有也已暂停/销毁，不执行旧事件 |
| 模拟点击成功，但复核时天气已恢复 | UI 操作成功可记录；业务不得车控，也不得显示车辆动作成功 |
...
> “徐洋，我需要确认天气第④类卡的 VUI 合同：Trigger 拉卡后怎样收到接收、排队、真正展示、失败、关闭和超时；卡片怎样向页面能力暴露确认/暂不按钮的显示文案、稳定元素 ID 和状态；用户手点或可见即可说模拟点击后，是否进入同一个按钮回调并关联同一次天气事件。卡片只回交互，不直接报车控成功。充电云端卡则请与 Advisor/Planner 一起拍板卡型、语音和点击回流。”

对左晨/字节语音与赛力斯页面注册方：

> “我不是请你们给 Trigger 返回一个 CONFIRM。我需要确认当前车机的完整可见即可说接入：谁解析天气卡这一最上层页面并生成文案、同义表达和元素 ID；动态热词怎样给在线/离线 ASR；匹配到一个或多个按钮后谁消歧；谁模拟点击；页面隐藏或刷新后旧映射怎样失效；模拟点击事件怎样写入对话历史。请给出现有接口、责任边界和天气卡最小接入示例。”

对端侧 Trigger 研发：

> “Trigger 不接原始 ASR 文本。你需要和 VUI 约定：天气卡的手点与模拟点击都回到同一个业务按钮事件，并携带当前运行事件。收到确认后你仍要重新读取任务状态与车况；UI 点击成功不是车辆动作成功，最后必须等真实状态回读。”

### 7.7.12 本节必须拿到的专项结论

| 待拍板问题 | 必须参加的人 | 本次最低产物 |
|---|---|---|
| “即时任务卡”是不是正式组件；与即时交互卡是子类、皮肤还是同一接口 | Task Service + 徐洋/VUI | 唯一术语、组件关系、正式调用边界 |
| 天气卡页面由谁解析并维护指令/元素 ID 映射 | 徐洋/VUI + 赛力斯页面方 | 页面注册方、当前顶层判断和映射生命周期 |
| 动态热词、在线/离线 ASR、匹配结果怎样交接 | 左晨/字节语音 + 页面注册方 | 输入输出、无结果/多结果、超时和去重时序 |
| 多候选由谁消歧，谁最终模拟点击 | 页面注册方 + 车企 UI | 消歧规则、模拟点击入口和失败返回 |
| 手点与模拟点击是否进入同一个按钮处理器 | 徐洋/VUI + 天气业务研发 | 同一业务回调、运行事件关联、输入来源标识 |
| 页面隐藏/刷新/被覆盖后旧映射怎样失效 | VUI + 页面注册方 + 语音 | 失效、恢复、旧元素 ID 拒绝规则 |
| 模拟点击事件由谁写入对话历史 | VUI/语音/对话上下文 Owner | “模拟点击事件”标记及下游消费规则 |
| UI 操作失败到底用 TTS、文字还是二者 | VUI + 语音产品 | 唯一失败反馈口径；不得误报业务成功 |
| 充电云端卡最终走 Static Advisor 还是三方调用链 | 徐洋/VUI + Advisor + Planner + 充电业务 | 卡型、语音路由、点击回流、上下文销毁 |
...
4. 端侧即时卡离线展示是否量产可用；
5. 可见即可说页面解析/映射、ASR/匹配、消歧、模拟点击、历史标记和页面失效接口及 Owner；
6. 复核、车控、状态回读及恢复天气流程。
...
| 用户交互 | 本地可见即可说：当前顶层元素匹配并模拟点击 | 在线 Planner：泛化语义与动态卡片上下文 | 卡片展示、手点/模拟点击回调、TTS、最终结果 |
...
| 可见即可说 ASR→匹配率 | 天气本地语音是否得到当前页面候选元素 | ASR/指令匹配问题 |
| 匹配→模拟点击成功率 | 候选元素是否被正确消歧且仍可操作 | 页面解析、元素 ID 或模拟点击问题 |
| 模拟点击→业务回调成功率 | UI 操作是否进入与手点相同的卡片按钮事件 | VUI/页面注册方/业务回调问题 |
...
| 语音匹配到隐藏页面按钮 | 顶层页面解析或旧元素映射失效缺陷 |
| ASR/匹配成功但没有按钮事件 | 页面注册方消歧、元素 ID 或模拟点击链缺陷 |
| 按钮动画成功但 Trigger 没收到确认 | VUI 控件事件/业务回调合同缺陷 |
...
| EDGE-04 | 卡片只排队未展示 | 不播 TTS、不算用户已看到，也不能成为可见即可说的当前页面 | VUI/页面注册方 |
| EDGE-05 | 正常展示后用户点击确认 | 回到同一次事件；复核通过才切模式 | VUI/Trigger/车控 |
| EDGE-06 | 天气卡是当前最上层页面，用户说“好的” | ASR→指令匹配→注册方消歧→模拟点击确认按钮→卡片正常回调同一事件；Trigger 复核后才执行 | 语音/页面注册方/VUI/Trigger |
| EDGE-07 | 卡片关闭、隐藏或过期后说“好的” | 旧页面映射和元素 ID 已失效，不模拟点击、不执行 | VUI/可见即可说 |
...
| EDGE-14 | 天气卡覆盖在另一个有“确认”按钮的页面上 | 只解析并匹配当前最上层天气卡；底层页面按钮不可被操作 | 页面注册方/可见即可说 |
| EDGE-15 | 当前页面有两个同名“确认”控件 | 匹配模块返回多个元素 ID；注册方按位置/上下文消歧，无法消歧则不点击 | 匹配模块/页面注册方 |
| EDGE-16 | 天气卡按钮文案或页面结构刚发生变化 | 指令映射动态更新；旧元素 ID 不能继续模拟点击 | VUI/页面注册方 |
| EDGE-17 | 免唤醒说出当前按钮指令 | 走纯离线 ASR；成功后按当前映射模拟点击 | 字节语音/端侧测试 |
| EDGE-18 | 唤醒后在线与离线 ASR 同时返回 | 按框架约定优先使用可执行的离线结果；无离线结果才等待在线，不重复点击 | 字节语音/页面注册方 |
| EDGE-19 | 分别用按钮原文、带“帮我选”前缀、长文本连续 4 字命中 | 分别覆盖精确/本地/模糊匹配；未达规则不操作 | 指令匹配/测试 |
| EDGE-20 | 单页面指令规模达到或超过 500 词边界 | 在原文能力边界内稳定工作；超限有明确限制/降级，不随机匹配 | 页面注册方/语音 |
| EDGE-21 | 可见即可说完成模拟点击 | 对话历史记录为“模拟点击事件”，不被下一轮 Planner 误解成普通 Query | VUI/语音/对话历史 Owner |
| EDGE-22 | 模拟点击成功，但 Trigger 复核失败或车控后状态未达到 | 可记录 UI 点击成功；最终业务结果仍为未执行/失败，绝不能显示车辆动作成功 | VUI/Trigger/车控 |
...
> “天气链从有效任务状态开始。Trigger 读取本地 VLM 天气/路面和车速、模式、雾灯，处理时效、防抖、去重和频控；命中后产生一次运行事件并请求第④类端侧自闭环卡。用户手点确认时，卡片直接产生按钮事件；用户说‘好的’时，语音不会直接回 Trigger，而是可见即可说先解析当前最上层卡片，生成按钮文案和元素 ID 映射，再经过字节 ASR、指令匹配、页面注册方消歧和 UI 模拟点击，最后由卡片走与手点相同的按钮回调。Trigger 收到确认后重新检查任务、天气、车速和目标状态，通过才调用赛力斯车控；最终以真实模式或灯状态收口。UI 模拟点击成功和车辆动作成功是两个结果，不能混。”
...
- 离线卡与展示 ACK；谁解析页面/生成元素 ID、谁接 ASR/匹配、谁消歧和模拟点击、页面变化怎样让旧映射失效；
- 卡片结果回哪个业务；
- 执行和状态回读。

### 第 6 段｜即时任务、即时交互卡和可见即可说（5 分钟）

> “先统一三个词：即时任务是能在当前轮次或较短时间完成的业务任务类型；即时任务卡是这类任务创建、触发或关闭时的瞬时反馈卡；即时交互卡是更通用的 VUI 临时容器。天气是长期存在、等条件命中的系统预设任务，命中后使用即时交互卡，并不等于它变成 Task Service 的即时任务。可见即可说又不是卡，它是语音操作当前最上层页面的一套框架。”

> “端侧天气按第④类自闭环卡：本地可见即可说把语音变成当前按钮的模拟点击，再由卡片把正常按钮事件回原天气业务。云端充电如果采用云端卡，卡片内容和上下文交给 Planner/动态选择能力，用户可以说更开放的话。两条链的共同点是卡片只承载界面和事件；不同点是本地链做 UI 指令匹配，云端链做上下文语义理解。”

现场先问清两组责任，不能只问徐洋一个人：

1. 徐洋/VUI：卡片拉起、展示、生命周期、稳定元素 ID、手点/模拟点击同路回调；
2. 左晨语音 + 赛力斯页面方：页面解析、动态热词、在线/离线 ASR、指令匹配、多候选消歧、模拟点击、历史事件；
3. 端侧 Trigger：只消费卡片业务事件，执行前复核，不直接消费原始 ASR；
4. Advisor/Planner：云端卡片上下文、开放语义、点击消费同步与上下文销毁。
...
| P0-04 | 天气本地卡 | 指定 VUI Owner；输出离线、拉卡/展示/失败/关闭/超时、元素 ID 暴露、手点与模拟点击同路回调 | 徐洋/VUI/端侧研发 |  |
...
| P0-17 | 天气可见即可说接入 | 指定页面注册与语音 Owner；输出顶层页面解析、动态指令/元素 ID、在线离线 ASR、匹配、消歧、模拟点击、历史记录和失效时序 | 左晨/字节语音/赛力斯页面方/VUI |  |
...
8. [《AI汽车｜可见即可说框架需求文档》](https://bytedance.larkoffice.com/wiki/AxYjwUI8wiFCXWkdNsDcRae0nCf)：当前最上层页面解析、指令与元素 ID、在线/离线 ASR、精确/模糊/本地匹配、多候选消歧、模拟点击、语音优先级和对话历史标记。
9. [《AIDV Task Service 与任务岛、即时交互卡技术方案》](https://bytedance.larkoffice.com/wiki/HzAtw3ABQif5c8kwKS8cL1x9nhf)：即时任务创建/触发/关闭通知、即时卡反馈、循环/长时任务与 AI Bar 任务岛、任务中心更新与启动拉取链。
10. [《赛力斯任务中心 PRD｜郝晓伟 9 月 1 日版》](https://bytedance.larkoffice.com/wiki/LUKKwMTNaiAd53kdmpkc5oILn5e)：系统预设任务纳入任务中心的最新范围、任务单元待补充、一期操作范围和任务列表推送/拉取。
11. [《AI汽车 FeatureList｜即时任务卡片定义所在行》](https://bytedance.larkoffice.com/sheets/PHXzs0YEmhomVQtvpcbcdsXqneg?sheet=69a063&range=ODU6ODU=)：将“即时任务卡片”定义为面向即时任务类型的瞬时、非生成式交互卡，作为本 PRD 区分术语的依据。
...
- 最新任务中心 PRD 纳入系统预设任务，只证明展示/管理范围方向；列表推送与启动拉取也不自动证明 Trigger 已获得运行所需的任务有效状态。
- “即时任务卡”和“即时交互卡”在资料中存在命名混用；在 Task Service/VUI 正式统一前不得写成完全同义。
- 可见即可说框架能力存在，不自动证明天气卡已经完成页面解析、ASR/匹配、模拟点击和业务回调接入。
