...
### 3.1 图一：一项云端系统预设任务从哪里开始、在哪里结束

```mermaid
flowchart TB
  subgraph PRE["上线前：先把任务定义和能力接好"]
    direction LR
    P1["① 产品定义任务<br/>场景、条件、交互、动作、验收"]
    P2["② 配置平台配置<br/>五组任务规则"]
    P3{"③ 发布检查<br/>能力与负责人齐全吗"}
    P4["形成可生效的<br/>任务配置版本"]
    PX["保留草稿<br/>指出缺少内容"]
    P1 --> P2 --> P3
    P3 -->|齐全| P4
  ...
  subgraph USE["用户使用：任务开启后等待合适场景"]
    direction LR
    U1["④ 用户在任务中心<br/>开启长期任务"]
    U2["⑤ 接入业务保存<br/>该车任务真实状态"]
    U1 --> U2
  end

  subgraph HANDLE["本次场景：判断、询问和执行"]
    direction LR
    T["⑥ Cloud Trigger<br/>读取配置、任务状态和车况"]
    H1{"⑦ 条件满足且<br/>未重复处理吗"}
    H2["⑧ 任务业务处理<br/>按需调用 DT / VQA"]
    H3["⑨ 按配置交互<br/>无需交互 / 告知 / 询问"]
    H4["⑩ 取得执行资格<br/>端侧复核、车控、状态回读"]
    H5["⑪ 业务收口<br/>更新卡片与记录"]
    HX["结束本次<br/>记录未继续原因"]
    T --> H1
    H1 -->|是| H2 --> H3 --> H4 --> H5
...
这张图分成三段。第一段解决“这项任务怎么被生产出来”；第二段解决“某辆车怎样真正开启”；第三段解决“某一次场景如何完成”。评审时必须先确认第一段，因为平台能力和业务接入没完成，后两段无法靠填写字段自动发生。

| 图中节点 | 谁负责、在哪里做 | 做什么 | 完成标志 |
|---|---|---|---|
| ① 产品定义 | 王泽民，在 PRD 与任务方案中 | 定义触发前提、进一步判断、交互、动作、异常和验收 | 任务不是只有“条件＋动作”两列 |
| ② 配置 | 产品在触发器场景配置平台操作；平台团队提供能力 | 引用已接入能力，形成五组配置 | 页面可以完整表达本次任务 |
| ③ 发布检查 | 配置平台 | 检查任务、能力、承接业务、结果分支和适用范围 | 缺项可定位；不能运行的配置不能发布 |
| ④—⑤ 用户开启 | 任务中心展示并转发；接入业务保存实际状态 | 把长期任务真正开启，并向用户回显结果 | 对指定车辆／账号可读取真实状态 |
| ⑥—⑦ 云端判断 | Cloud Trigger | 读取同一配置版本、任务状态和云端车况，判断事件、条件、有效性与重复 | 满足时只发一次业务事件；否则记录原因 |
| ⑧—⑨ 后续处理 | 系统预设任务业务 | 按配置决定是否进一步识别、是否询问；业务自己注册卡片 | 取得明确业务结果和有效用户选择，或明确结束 |
| ⑩—⑪ 执行收口 | 端侧执行、赛力斯车辆能力、任务业务 | 复核最新条件、请求车控、读取实际状态并更新结果 | 真实状态达到目标才算业务成功 |
