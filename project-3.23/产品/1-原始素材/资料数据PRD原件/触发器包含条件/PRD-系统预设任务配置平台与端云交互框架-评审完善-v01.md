...
| 使用者 | 现在需要解决的问题 | 本期希望得到的效果 |
|---|---|---|
| 车主 | 合适场景出现时仍需主动操作；不知道推荐是否真的执行 | 一次开启，适时服务；询问型动作先确认；结果以实际车况呈现 |
| 任务产品／配置者 | 只写条件和动作，无法表达识别后再询问、拒绝不执行 | 在同一场景编辑页配置完整处理过程，并看到缺项 |
| 能力接入研发 | 信号、卡片、语音、车控分别有人做，但交接可能无人承接 | 每项配置有消费模块，每次事件有完整业务处理人 |
| 测试和问题定位人员 | “触发成功”被当成“车辆成功” | 能定位没有触发、识别终止、卡片未展示、未确认、复核拦截或结果未知 |

现有平台基础沿用原稿及此前页面核验：基础信息、适用范围、动态／静态条件、条件组、下游动作、次数与间隔、触发记录。本文逐项提出扩展，不把未核验的字段称为当前已上线。

### 1.3 目标与验收结果

本期要验证：充电任务能被五组配置完整表达；端侧天气任务能闭环确认和执行；儿童锁、后视镜的策略分歧能被明确决策，不能带着互斥逻辑开发。配置、运行、车辆结果应能关联同一任务及同一次处理。

价值在于复用已接入的能力和处理阶段，减少逐场景重复接入，并保留用户控制权。云端适合进一步调用模型和工具；端侧适合使用本地状态、固定规则及本地交互。划分依据是本任务需要的数据和已接入能力，不是“有网走云、无网自动转端”。车控最终仍在车辆侧执行，端状态负责告诉业务实际变成什么。

本次方案评审的输出是：确认平台改造范围、原子能力清单、模块交接、四项任务规则和待确认项的承接人。随后才能确认开发拆分与排期。本稿不承诺未经测试的成功率、时延或节省比例。
...
| 执行与结果 | 业务申请；端侧复核；车控动作；端状态回读 | 车辆真实结果给业务，再更新卡片与记录 | 真正达到目标才算业务成功 |
...
| 业务→主动推荐／DT／VQA | 检查目标、当前场景、允许输入、请求关联 | 当前能力能否完成约定判断 | 四类识别结果、观察时间和关联；受理不算判断完成 |
...
#### 6.2.1 确认方案的完整演示（仅用于比较方案，不代表已批准）

假设本次选择“切D启动、后排目标儿童、双侧锁关闭、卡片确认”的评审方案，车辆其他限制满足。双侧只是演示目标，正式侧别待会签。

| 步骤 | 处理与依据 | 输出及用户感受 |
|---|---|---|
| 1 | 状态业务让该车任务许可生效；端侧或云端按选定部署开始监听 | 不把开启任务当作立即锁门 |
| 2 | 收到非D→D事件；读取成员类型、后排范围、左右锁及必要门状态 | 任一关键数据未知则不执行，记缺少依据 |
| 3 | Trigger确认目标成员成立、目标锁未开且没有重复 | 发“建议开启目标侧儿童锁”事件；不直接锁门 |
...
若选静默方案，跳过第4、5步，并将第6步的“用户明确确认”替换为“已会签的静默策略允许本次动作”；其他数据有效性、动作范围、端侧复核和真实回读不能省略。若选A方案，则由非P下的成员／状态变化触发检查，不能沿用B方案“仅切D一次”的覆盖范围。上述决策未定时可评审，但该任务不能作为已具备唯一运行规格进入正式发布。
...
| 5. 发起判断 | 业务按配置交主动推荐／DT：“请判断本次约定观察范围内是否看见充电设备，并返回结果” | 当前能力支持该观察；请求仍属于这次停车 | 等待识别；Query不写用户已授权，也不直接命令开盖 |
...
| 真实目标达成 | 可信模式／灯／锁／口盖／加热状态达到目标 | 提前已达应记无需执行；不冒称本任务导致 |
...
|---|---|---|---|---|
| D01 | 开关按车辆还是账号；保存成功与监测生效的用户口径；Task Service是否承接 | 王泽民、郝晓伟、状态／Trigger研发 | 所有任务许可、跨账号隔离、同步关闭 | 状态接口与任务中心交互评审前 |
| D02 | 平台、端云业务、卡片、执行、测试的真实研发认领 | 王泽民、汪帅／项目侧及模块对接人 | 工作拆分有人承接，不是新增产品Owner | 开发拆分及排期前 |
| D03 | 哪些能力本期可配置；端侧是否仅规格管理；发布消费与在途版本策略 | 韩杰、Trigger、任务业务及能力团队 | 本期平台发布范围和版本一致性 | 平台方案／能力目录会签前 |
...
| D09 | 充电先识别后确认是否批准；观察范围、设备定义、图像来源、端侧取DT结果交接 | 王泽民、张弛、丁彬、刘多、端侧接入 | 识别输入、调用顺序、授权语义 | 充电能力与交互专项前 |
...
本稿依据上传HTML全文及6张图，结合用户此前明确的模块边界完成；旧云端V07仅作为页面细化写法参考，不能证明现平台已实现。本轮未访问或修改飞书和真实后台。

多Agent审核分别从前端／平台、后端／Trigger／任务中心、即时卡／语音／四场景测试视角提出问题；主稿完成后再次进行交叉检查。角色模拟用于提前发现遗漏，不代表真实业务方已批准，更不能替代实际车型联调与安全评审。具体发现和处理见单独的《评审问答与Review记录》。
...
- [独立评审问答与Review记录](系统预设任务-评审问答与Review记录-v01.md)：面向逐图说明、研发追问和本轮审核结果，不混入正式需求正文。
