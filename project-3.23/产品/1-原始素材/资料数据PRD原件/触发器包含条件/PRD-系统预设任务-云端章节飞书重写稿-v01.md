...
| 丁彬 | 充电视觉能力对接人 | 对齐 VQA 输入、设备定义、观察结果和有效期 | 画面来源、FOUND / NOT_FOUND / UNKNOWN / ERROR 口径、VQA 研发 | 能力对接明确；研发 DRI 待确认 |
...
| KR4：覆盖关键反向 Case | 任务关闭、状态过期、重复停车、DT 非 FOUND、卡片未展示、拒绝 / 超时、确认后离开 P 挡、车控失败等均不发生错误开盖 |
...
```mermaid
flowchart TB
    A[产品定义一项系统预设任务<br/>用户价值、范围、条件、交互、动作、验收]
    B[配置平台选择已接入原子能力<br/>补齐任务关联、规则、处理、交互、策略]
    C{配置完整且能力可用吗}
    C0[阻止发布<br/>指出缺失能力、Owner或规则]
    D[任务中心展示任务<br/>用户开启或关闭]
    E[任务接入业务保存真实状态<br/>把结果回显任务中心]
    F{Cloud Trigger读到任务已开启<br/>且关键状态有效吗}
    F0[不产生新业务事件<br/>等待状态恢复或下一次场景]
    G[Cloud Trigger判断启动事件、当前条件<br/>防抖、去重、频控和单次边界]
    H{本次是否满足启动条件}
    H0[记录不触发原因<br/>继续等待]
    I[Cloud Trigger发出一次业务事件<br/>不拉卡、不车控]
    J{是否需要进一步判断}
    K[原业务直接形成处理建议]
    L[主动推荐 / Advisor / DT / VQA<br/>补充识别或业务判断]
    M{结果是否明确、有效<br/>且仍属于当前场景}
    M0[原业务结束本次<br/>不出执行卡、不车控]
    N[原业务决定本次交互方式]
    O{是否需要用户确认}
    P[按已会签静默策略<br/>形成端侧执行申请]
    Q[原业务注册并拉起即时交互卡<br/>配置文案、TTS、按钮、回调和有效期]
    R{卡片真正展示且用户明确同意吗}
    R0[拒绝 / 关闭 / 超时 / 过期<br/>原业务结束本次]
    S[原业务形成端侧执行请求<br/>关联当前任务、场景、判断和授权]
    T{端侧最新任务和车况<br/>仍允许且未重复吗}
    T0[不调用车控<br/>返回不执行原因]
    U[赛力斯车控执行目标动作]
    V[读取车辆真实状态]
    W{是否达到目标状态}
    X[原业务更新卡片和记录<br/>成功 / 失败 / 暂不能确认]
    Y[结束本次处理<br/>长期任务仍开启则等待下一次]
...
2. Cloud Trigger 的终点是一次业务事件，不是卡片或车控。
3. 模型判断的终点是业务结果，不是用户授权。
4. 原业务是云端后半段的唯一收口方；卡片和 Planner 都是原业务调用或使用的能力。
5. 最终成功由车辆真实状态决定，前面任意环节成功都不能代替它。
...
| 8. 返回识别结果 | 端侧 VQA、DT、充电原业务 | 观察请求、当前画面和任务上下文 | VQA 返回观察事实；DT 形成 FOUND、NOT_FOUND、UNKNOWN 或 ERROR，并带观察时间 / 适用场景 | 业务可消费的判断结果返回充电原业务 | 非 FOUND、过期或不属于当前停车时，不进入询问 |
| 9. 原业务注册并拉起卡片 | 充电原业务 | 有效 FOUND、当前停车和最新任务状态 | 再核对建议仍有效；准备文案、TTS、确认 / 取消按钮、点击回调、Planner 语音上下文、有效期和更新方式；调用即时交互卡 | 一次明确关联本次建议的卡片请求 | 原业务未认领、建议失效或注册失败时结束本次；Planner 和 Trigger 不代替拉卡 |
...
| DT 输出业务结果 | FOUND、NOT_FOUND、UNKNOWN、ERROR；结果与本次请求关联 |
| 原业务判断 | 只有 FOUND、未过期、仍属于当前停车、任务未关闭时才可形成询问建议 |
| 明确禁止 | DT / VQA 直接创建卡片、直接判断用户已同意或直接下发开盖 |
| 失败处理 | NOT_FOUND / UNKNOWN / ERROR / 过期均结束本次，不出开盖确认卡；真实原因进入记录 |
...
stateDiagram-v2
    ...
    进一步判断 --> 无动作结束: NOT_FOUND / UNKNOWN / ERROR / 结果过期
    进一步判断 --> 待注册卡片: FOUND且仍属于当前停车
...
| DT / VQA | NOT_FOUND | 结束本次，不出开盖确认卡 | 把未看见说成没有设备，或继续开盖 |
| DT / VQA | UNKNOWN / ERROR / 超时 | 结束或有限重试；没有明确结果不询问开盖 | 将调用完成当 FOUND |
...
| DT 返回 FOUND | 仅形成可以询问的业务结果，不直接开盖 |
| DT 返回 NOT_FOUND / UNKNOWN / ERROR / 过期 | 不出开盖确认卡、不执行 |
...
| DT 结果分布 | FOUND / NOT_FOUND / UNKNOWN / ERROR / 超时分别统计 | 判断视觉能力、场景和超时策略 |
...
| P0-07 | VQA 观察能力和结果口径 | 没有可信四类结果，无法决定是否询问 | 丁彬、VLM / AI Box、模型团队 | 视角 / 画面、任务 ID、观察时间、FOUND 等正式含义和准确率验收 |
