...
flowchart LR
  ...
  subgraph B["② 用户启用与触发"]
    direction TB
    B1["任务中心<br/>用户开启任务"]
    B2["任务状态承接业务<br/>保存并提供真实开关"]
    B3["Cloud Trigger<br/>读取开关、配置和上行车况"]
  ...
  subgraph C["③ 业务处理与交互"]
    ...
    C2["业务得到有效结果<br/>按配置决定是否询问"]
    C3["系统预设任务业务<br/>注册卡片并申请展示"]
    CV["即时交互卡 / VUI<br/>展示卡片、按钮和 TTS"]
    C4["点击按钮<br/>结果直接回业务"]
...
flowchart LR
  ...
  subgraph E["核心改造｜同一个新建 / 编辑页面"]
    direction TB
    E1["① 基本信息区：扩展<br/>保留车型版本；新增任务关联、承接业务"]
    E2["② 条件区：扩展<br/>保留条件组；新增数据时效、停车去重"]
    E3["③ 动作区：扩展为业务处理<br/>承接业务、判断能力、结果分支"]
    E4["④ 新增交互与后续动作区<br/>卡片、TTS、按钮；确认后动作与回读"]
    E5["⑤ 运行策略区：扩展<br/>保留次数间隔；新增失效、关闭和重试策略"]
    E1 --> E2 --> E3 --> E4 --> E5
  end
  subgraph R["出口｜发布与查看结果"]
    direction TB
    R1["检查配置<br/>缺项定位回填；完整后允许发布"]
    R2["发布版本并确认生效<br/>Trigger 读规则，业务读后续处理"]
    R3["扩展触发记录 / 单次详情<br/>触发、识别、卡片、回答、车辆真实结果"]
    R1 --> R2 -->|运行后查看记录| R3
  end
  L -->|进入编辑页| E
  E -->|填写完成| R
