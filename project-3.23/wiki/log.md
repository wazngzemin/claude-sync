...
| 2026-07-13 | ingest | 7月10日sls端状态接入context,接入触发器需求评审.md（65min韩杰主持） | 创建 summary: meeting-0710-sls-signal-context-trigger-review（九节：信号中枢三表/并行→串行漏斗/事件日志链路/端侧触发器定位争议/信号协议改动/730三需求/争议与未决/行动项/对泽民影响）。创建 concept: signal-hub（信号中枢三表架构+与旧架构对比+四个设计理由）。更新 entity: trigger-engine（纳入信号中枢框架/新旧架构图/触发事件+条件拆分/730需求/端侧定位争议）。更新 concept: trigger-condition-taxonomy（触发事件+条件二分法/VLM类型新增/串行并行延时复合组合）。更新 index.md |
| 2026-08-18 12:00 | ingest | 飞书群聊截图：触发器条件任务重复触发讨论（施佳杰/王泽民/郝晓伟） | 转录至 raw/articles/chat-0818-trigger-repeat-trigger-discussion.md；创建 summary: chat-0818-trigger-edge-trigger-discussion（现象：电量<30%持续期间按CD反复提醒；根因：电平触发只判断当前是否满足；定论：边沿触发 one-shot，阈值穿越瞬间 push 一次，条件回落再满足才重触发，CD只降频不去重；行动项：泽民找刘畅对齐改造）；更新 entity: trigger-engine（新增"触发语义定论：电平→边沿"章节）；更新 concept: trigger-condition-taxonomy（数值阈值=边沿语义补充）；更新 index.md |
| 2026-09-03 17:05 | query | cot-fast-slow-thinking-architecture + cockpit-planner-onepage | 核验并解释 FC：在当前 Planner 语境中指 Function Call，即模型选择外部工具并生成调用参数；同时区分“生成调用”与下游真实执行结果。 |
| 2026-09-03 17:12 | query | 飞书：座舱Planner OnePage + 基于COT的新快慢思考架构实验 + FC协议表 + 多指令准确率方案 | 回源核验内部 FC 口径：既指 Function Call 机制，也指 SFT/RAG FC 模型层；其产物是具体函数及结构化参数，简单任务由下游 SFT FC 承接，复杂任务可先由 Director/Planner 推理或改写后再进入 FC。 |
| 2026-09-03 17:15 | query | 飞书：座舱大模型AI助手产品PRD + 座舱Planner探索 | 对比规则、传统 NLU/NLP、FC 与 Planner：规则/句法RAG负责前置确定性匹配，NLU负责意图和槽位理解，FC负责形成可执行函数与参数；当前采用仲裁分流的混合漏斗，三者可同时存在。 |
| 2026-09-03 17:34 | query | 飞书：离线LLM指令解析 + 离在线仲裁 + 离线指令FC协议 + 多指令准确率提升技术方案 | 回读原始PRD和协议后纠偏：规则/缓存、离线FC模型、云端Planner是仲裁选择的不同处理路径，并非依次串联的三个步骤；FC在内部既指结构化函数调用协议，也常代指从自然语言生成该协议的SFT/RAG模型。离线FC协议页签标注为old，仅用于说明术语与格式，不能当作当前全量上线证据。 |
| 2026-09-03 17:42 | query | 本地Wiki + 飞书《记忆系统》v1.8（2026-08-31） | 核验账号与记忆的最新关联：记忆单位以“单车×单豆包账号”为最大数据边界；记忆开关、记忆内容随车辆和账号切换；Face ID用于单位内的人物/在场场景筛选；SLS共享乘客可在主账号用车时同步补充其账号下对应记忆单位。区分正式PRD规则、界面规划和仍需结合Face ID判断的执行边界。 |
| 2026-09-03 17:43 | query | 飞书：离线LLM指令解析 + 工具库V1.0 + SLS第二批FC需求评审逐字稿 + AICAR多指令准确率方案 | 核验离线FC与SFT FC：前者按端侧部署/离线链路命名，后者按监督微调实现方式命名；当前项目资料中离线FC作为端侧LLM与白名单能力管理，SFT FC主要是Director下游工具内的云端FC生成分支，二者可共用函数槽位协议但不等于同一模型。多指令方案可跳过SFT改走RAG；离线模型的具体训练方式文档未确认。 |
| 2026-09-03 17:50 | query | 浏览器+飞书：《离线指令FC协议》离线FC（New）+《座舱大模型主线全域FC协议》控制合辑（0829）+ 9月1日产品内审逐字稿 | 纠正并核验端侧FC链路：本地高频指令缓存 > 本地白名单FC > 云端Planner > 本地非白名单FC；缓存命中是预配置query直接取得唯一意图槽位/FC结果，不等于白名单命中。离线New共582行，主线0829共2434行，两者复用函数槽位结构，离线表增加离线白名单字段并缩小能力范围。 |
| 2026-09-10 | query | 产品/1-原始素材/会议纪要/专家知识-现状及计划讨论.md | 基于内嵌原始文字记录总结专家知识现状、点面体知识结构、Dynamic Advisor与Planner分工、自学习路线、底层感知依赖，并拆解讨论深入的原因和可复用方法论；产出产品/codex/Meeting-Summary-2026-09-09-专家知识现状及计划讨论.md。 |
| 2026-09-11 16:40 | query | 飞书《记忆系统》v1.8 + 8·19 产品内审原始文字记录 | 使用 Codex 内置浏览器逐张核对 17 张记忆画板与 39 条评论，结合逐字稿区分现状、评审方向、最新版 PRD 与 TBD；产出记忆模块深读 v02 及 GPT Image 全链路图。 |
| 2026-09-14 21:10 | query | 飞书：记忆系统 v1.8、AI汽车记忆方案、记忆待讨论内容、POI 同步方案、8·19 产品内审逐字稿、AI 汽车测试周会 | 重新按记忆模块全资料链取证，核对正文、总流程/隐私画板、评论、逐字稿和测试数据；区分已确认、进行中、delay、未开始及无权限证据缺口；产出 Feishu-记忆模块全源深读与会议插话指南-v03.md 和 GPT Image v02。 |
