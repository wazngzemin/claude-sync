...
> 文档目的：先用产品能讲懂的业务主线说明“用户如何管理任务、系统如何发现场景、为什么分端侧与云端、何时询问、谁执行、怎样才算成功”；再把数据合同和技术时序作为研发专项材料展开。
>
> 配套可点击总图：[系统预设任务完整框架架构图-v02.html](./系统预设任务完整框架架构图-v02.html)  
> 说明：本 PRD 第 7.2.1 节是产品评审唯一主讲总图；配套 HTML 和后续研发图只用于被追问时展开，不作为产品开场主线。
...
| V7.0 | 新增产品经理版业务总图和八阶段责任表；把六层图、30 步主链、状态机明确降级为研发专项展开材料；主图全部改用用户与业务语言 | 不删 v06 技术细节，只调整讲解层级 |
...
3. 再讲第 7.2.1 节：产品业务总图；这是本次唯一主讲总图；
4. 再讲第 7.5 节：任务中心怎样把真实状态交给 Runtime；
5. 再讲第 7.6.2 与 7.7.4 节的两条案例总流程；六层图、状态机、30 步链和逐步 JSON 只在研发追问时展开；
6. 再讲第 7.8.7 节：四任务结论；
...
**系统预设任务，是产品提前定义好、用户通过任务中心查看并在本期提供相应控件时提交操作意图、运行侧长期等待条件、每次命中后按端侧或云端链路完成判断，并最终由端侧安全执行和读取真实车辆结果的一类长期自动化能力。**
...
## 7.1 研发专项：九类公共合同对象｜产品初次阅读可跳过

> 本节用于研发把业务语义落成数据合同，不是产品业务主线。产品评审先看第 7.2.1 节；只有研发追问“每一跳如何关联”时再展开本节。

<details>
<summary>展开查看九类公共合同对象</summary>
...
> **产品评审只主讲第 7.2.1 节。**这张图回答用户经历什么、业务为什么分端云、什么时候询问用户、谁最终执行、怎样才算成功。后面的六层图、30 步主链和状态机是研发专项材料，不需要在产品开场时逐个解释。

### 7.2.1 产品业务总图｜评审主讲这一张

> **【目标产品要求】**以下是一套需要在专项评审中拍板的统一业务闭环，不代表任务状态同步、端云分工、卡片结果回流和车辆真实状态反馈已经全部实现。图中明确标为“待确认”的内容，在负责人、方案和验收口径确定前不得作为现状或研发承诺。

#### 用户会经历什么

~~~text
用户在任务中心看见一项系统预设任务
→ 若本期提供相应控件，任务中心接收用户的开启、关闭等操作意图（具体范围待确认）
→ 负责保存任务状态的业务返回真实处理结果（负责人和同步方式待确认）
→ 运行侧读取这辆车的任务是否真的有效
→ 系统长期等待合适场景
→ 基础条件稳定成立后，先建立本次唯一处理记录
→ 按任务已确认的端云方案，由端侧完成判断或由云端补充判断
→ 按任务约定：无感处理、只提醒，或者询问用户
→ 只提醒的分支告知后结束，不产生车辆动作
→ 需要执行车辆动作的分支，以车辆真实状态形成成功、失败或暂时无法确认结果
→ 车辆动作结果最终展示在卡片还是任务中心、由谁回传，待专项确认
~~~
...
~~~mermaid
flowchart TB
    A["① 产品提前定义任务<br/>什么场景、是否询问、做什么、怎样算成功"]
    B["② 发布这项任务<br/>目标：任务中心可展示，已确认的端/云方案可运行"]
    C["③ 任务中心接收用户操作意图（若本期有控件）<br/>状态业务保存并返回真实结果；范围与负责人待确认"]
    D{"④ 运行侧读到的任务真的有效吗？<br/>状态不清楚也不能运行"}

    E["⑤ 系统持续等待场景<br/>观察天气、车况、视觉等变化"]
    I0{"⑥ 基础场景条件稳定成立吗？<br/>数据新鲜、持续稳定、没有重复处理"}
    R0["⑦ 建立本次唯一处理记录<br/>先记录，再调用模型、出卡或执行"]
    F{"⑧ 读取任务发布前已确定的判断方案<br/>端云方案未确认的任务不得发布"}
    G["端侧完成判断<br/>规则明确、数据在车上、要求快或弱网可用"]
    H["云端补充判断<br/>需要模型、视觉工具或更多上下文"]
    I{"最终判断结果明确、未过期<br/>而且属于本次场景吗？"}

    J{"⑨ 这次要怎样与用户交互？"}
    K["无需每次询问<br/>仅限已完成业务与安全会签的任务"]
    L["只做提醒<br/>卡片或语音告知后结束，不执行车辆动作"]
    M["必须询问<br/>即时交互卡展示问题并收集点击或语音"]
    N{"用户对本次动作明确同意吗？<br/>关闭、超时或没听懂都算没有同意"}

    O["⑩ 端侧执行前重新检查<br/>任务仍开启、车况仍满足、本次允许仍有效"]
    P{"此刻还允许安全执行吗？<br/>任一条件变化都停止"}
    Q["⑪ 赛力斯车辆能力执行动作"]
    R["⑫ 读取车辆真实状态<br/>不能只看请求是否收到"]
    S{"车辆真的达到目标了吗？"}

    T["⑬ 形成本次真实结果：成功"]
    U["⑬ 形成本次真实结果：失败或暂时无法确认<br/>不能把请求已收到说成成功"]
    W["【待确认】结果反馈到哪里、由谁回传<br/>即时交互卡或任务中心"]
    V["本次结束并进入冷却<br/>长期任务继续等待下一次场景"]
    X0["停止运行<br/>等待任务状态恢复"]
    X1["本次不打扰用户<br/>继续观察场景"]
    X2["本次结束<br/>不执行车辆动作"]
    X3["停止并反馈原因<br/>不调用车辆能力"]
    ...
    classDef prepare fill:#E8F1FF,stroke:#4F7FF0,color:#17356B;
    classDef edge fill:#E8F8F1,stroke:#2EA778,color:#175E48;
    classDef cloud fill:#F1EAFE,stroke:#8661D1,color:#50348D;
    classDef interaction fill:#FFF5DF,stroke:#E39B30,color:#704A0E;
    classDef execution fill:#FFE9E5,stroke:#E06B55,color:#7A2E22;
    classDef stop fill:#F3F4F6,stroke:#9AA1AC,color:#4B5563;

    class A,B,C,D,I prepare;
    class E,I0,R0,F,G edge;
    class H cloud;
    class J,K,L,M,N interaction;
    class O,P,Q,R,S,T,U,W,V execution;
    class X0,X1,X2,X3 stop;
~~~
...
> **产品先定义一项长期任务；若本期任务中心提供相应控件，用户可以提交操作意图，真实状态由谁保存和怎样同步仍需拍板。基础场景条件稳定成立后，系统先建立本次唯一处理记录，再读取这项任务在发布前已经确定的判断方案：规则明确且要求快速/弱网工作的由端侧完成，需要模型、视觉或复杂上下文的由云端协作；端云方案未确认的任务不得发布。随后按任务策略决定是否询问用户；无论前面走端还是云，最终都回端侧重新检查、执行车辆动作，并以车辆真实状态作为成功标准。**

#### 八个业务阶段分别由谁负责

| 业务阶段 | 用户看到什么 | 主责任方 | 负责什么 | 失败时怎么办 |
|---|---|---|---|---|
| 1. 定义任务 | 暂时看不到 | 产品 + 配置/发布平台 | 写清场景、规则、交互、动作、成功标准和适用车型 | 任一关键能力或负责人不清楚，不发布 |
| 2. 用户管理 | 看见任务；若本期提供相应控件则可提交操作意图 | 任务中心 + 负责保存任务状态的业务（支持范围、负责人、同步方式待确认） | 任务中心承接用户意图；保存状态的业务确认这辆车的任务是否真的有效 | 保存失败则页面回滚；状态不明不运行 |
| 3. 等待与初筛 | 通常无感 | 端侧或云端触发器 | 持续观察天气、车况、视觉等基础信息，确认数据新鲜并稳定满足 | 条件不满足或数据不可靠就继续等待 |
| 4. 避免重复处理 | 通常无感 | 被指定的唯一触发方 | 基础条件稳定成立后，先为本次场景建立唯一处理记录，再调用模型、出卡或执行 | 发现已有同一场景在处理，则复用或结束，不再新建一轮 |
| 5. 端云判断 | 通常无感 | 端侧触发器，或负责云端判断的业务 | 按任务发布前已确认的端云方案完成判断；无论走端还是云，最终结果都必须明确、未过期并属于同一次处理记录 | 无法判断、结果过期或端云归属未确认时，本次不继续；未确认任务不得发布 |
| 6. 用户交互 | 无感、只提醒，或看到确认卡 | 发起该任务的业务模块（原业务）+ 即时交互卡；语音按端侧/云端链路处理 | 原业务决定问不问、问什么；卡片负责展示并收集用户选择 | 未明确同意、卡片关闭或超时，不执行需要确认的动作 |
| 7. 执行车辆动作 | 可能看到“正在执行” | 端侧执行 + 赛力斯车控 | 重新检查最新任务和车况；仍安全才调用车辆能力 | 条件变化、请求过期或安全门不通过，停止并返回原因 |
| 8. 结果反馈 | 只提醒分支看到告知结果；车辆动作分支目标是看到成功、失败或暂无法确认 | 告知结果由卡片/VUI负责；车辆结果由赛力斯状态域 + 原业务负责；最终展示位置和回传负责人待确认 | 只提醒时告知后结束；执行车辆动作时读取真实状态，形成成功、失败或暂时无法确认 | 只提醒不得进入车控；车辆状态读不到就标记为暂时无法确认，不能把“请求已收到”说成成功 |

#### 端侧和云端到底怎样区分

| 业务问题 | 放端侧判断 | 需要云端协作 |
|---|---|---|
| 判断规则 | 条件明确，例如 AND/OR、阈值、状态变化 | 需要理解语义、识别物体或综合多种上下文 |
| 数据在哪里 | 天气、路面、车速、挡位等数据已经在车上 | 需要 DT/VQA、模型、历史或更多云端工具 |
| 响应要求 | 场景窗口短，要求马上判断 | 可以容忍联网、排队和推理时间 |
| 网络要求 | 弱网或无网也应继续工作 | 明确依赖在线能力 |
| 典型例子 | 天气/路况保护 | 识别充电设备后打开充电口盖 |

共同底线只有一句：

> **端侧和云端只决定“在哪里完成判断”；最终车辆动作、执行前安全检查和真实状态确认都在端侧。**

#### 三种交互方式先用人话讲

1. **无需每次询问**：任务已经完成业务与安全会签，场景成立后可以直接进入端侧执行前检查。儿童锁、后视镜目前只能算候选，未会签前不能按这条路开发。
2. **只做提醒**：用卡片或 TTS 告诉用户发生了什么，提醒完成后本次结束，不能偷偷进入车控。
3. **必须询问**：卡片真正展示后，收集用户针对本次动作的点击或语音；拒绝、关闭、超时、没听懂都不执行。

卡片的边界也只需要记住一句：

> **发起该任务的业务模块（下文简称“原业务”）决定为什么问、问什么以及用户同意后做什么；即时交互卡只负责把问题展示给用户并把本次选择交回来，它不判断场景，也不直接控制车辆。**

#### 这张业务图仍有三项必须在会上确认

- **任务中心**：系统预设任务一期支持开启、关闭、删除中的哪些操作；谁保存真实任务状态；状态怎样提供给运行侧。
- **充电任务**：第一次场景由端侧还是云端唯一发起；最终用哪种卡；点击和语音回到哪里。
- **儿童锁与后视镜**：当前“无卡、无 TTS”只是候选方向；安全会签、正式规则、人工覆盖和端云归属未完成前，不能承诺静默车控。

#### 评审现场可以直接照读

> “这张图先不讲接口，只讲业务。产品先定义一项长期任务；如果本期任务中心提供相应控件，由任务中心接收开启或关闭等操作意图，再由负责保存状态的业务返回真实结果。具体支持范围、谁保存状态、怎样同步，需要今天拍板。场景出现并且基础条件稳定成立后，系统必须先建立本次唯一处理记录，再读取这项任务发布前已经确定的判断方案：本地数据足够、规则明确、要求快或弱网可用的由端侧完成判断；需要模型、视觉工具和更多上下文的由云端协作；端云方案未确认的任务不得发布。无论走端还是云，判断结果都必须明确、未过期并属于本次场景。接着按任务策略决定是无需每次询问、只提醒，还是必须询问用户。只提醒的任务告知后就结束，不执行车辆动作；需要执行的任务，只有已经完成静默安全会签，或者用户对本次动作明确同意，才会回端侧重新读取最新车况。最后由赛力斯车辆能力执行并读取真实状态，真实状态达到目标才算成功。车辆动作必须形成成功、失败或暂时无法确认的结果；最终展示在卡片还是任务中心、由谁回传，仍需专项确认。”

### 7.2.2 研发责任与数据合同展开图｜产品评审默认不讲

> 这一图回答研发模块怎样分工，包含产品会上不需要主动解释的技术对象。只有研发追问“状态怎样同步、怎样防止端云双发、凭什么允许执行”时再展开。交互式版本见文首 v02 HTML。

<details>
<summary>展开查看六层研发协作图</summary>

> 读图方法：从上往下看一次任务怎样流动；横向看每层模块边界。橙色/TBD 内容不是现网事实，而是本次需要认领 Owner 和技术产物的目标产品合同。
...
### 7.2.3 研发专项：从后台到车辆真实完成的 30 步主链

> 产品评审不逐步讲这张图。它用于研发确认每一跳的数据、异常和责任，只有被追问“具体在哪一步传什么”时再展开。

<details>
<summary>展开查看 30 步研发主链</summary>
...
### 7.2.4 研发专项：一次运行的详细状态机

> 产品只需要理解“等待场景 → 本次处理 → 交互/执行 → 真实结果 → 冷却”。下面的英文状态用于研发对齐，默认收起。

<details>
<summary>展开查看详细状态机与状态责任表</summary>
...
| 业务门 | 谁负责 | 只问一个问题 | 通过后做什么 | 不通过怎么办 |
|---|---|---|---|---|
| ① 任务有效门 | 真实状态方 + 运行侧 | 用户的任务真的已经开启，并且适用于这辆车吗？ | 开始长期等待场景 | 状态关闭或不清楚时不运行 |
| ② 场景成立门 | 端侧或云端 Trigger | 当前条件可靠吗、稳定吗，而且不是同一场景重复处理吗？ | 建立本次唯一处理记录；需要时调用模型 | 条件不满足、数据过期或无法判断时继续等待 |
| ③ 交互允许门 | 原业务 + 即时交互卡 | 这次是已经会签的无感任务、只提醒，还是已经获得用户本次明确同意？ | 无感任务或明确同意才进入执行前检查；只提醒直接结束 | 拒绝、关闭、超时、歧义或没有安全会签时不执行 |
| ④ 当前安全门 | 端侧执行 + 赛力斯车控/状态 | 真正执行前，任务和最新车况是否仍然允许做？ | 调用车辆能力并读取真实状态 | 任一条件变化立即停止；真实状态未达到目标不能报成功 |

一句话记忆：**先确认任务真的开启，再确认场景真的成立，再确认本次允许继续，最后确认车辆此刻还能安全执行。**

## 7.3 研发专项：每个模块的完整责任边界

> 产品主线已经在第 7.2.1 节说明。下面的七问责任表供各研发 Owner 认领输入、判断、输出和失败处理，产品评审默认收起。

<details>
<summary>展开查看模块完整责任表</summary>
...
~~~mermaid
flowchart LR
    U["① 用户在任务中心操作<br/>想开启或关闭任务"]
    TC["② 任务中心传递操作意图<br/>它自己不判断任务是否已生效"]
    S["③ 负责保存任务状态的业务处理<br/>具体由谁负责仍待确认"]
    OK{"④ 保存成功吗？"}
    UI["成功：任务中心更新真实状态<br/>运行侧同步开始或停止等待场景"]
    FAIL["失败：任务中心回滚并提示<br/>Trigger 不能按新状态运行"]
    TR["⑤ Trigger 按真实状态运行<br/>任务关闭或状态不明就不触发"]
...
一句话：**任务中心负责接住用户操作，保存任务状态的业务负责确认是否真的生效，Trigger 只按最终真实状态运行。**当前仍需晓伟和任务系统确认这个业务 Owner 以及正式同步方式。

<details>
<summary>研发追问时展开：状态同步的目标技术语义</summary>
...
> “现在的问题不是四条规则没人写，而是大家还没有一张共同的业务图。任务中心、Trigger、卡片、云端判断和车辆执行各有材料，但用户开启以后怎样一路走到车辆真实完成，没有连成一条线。今天先把业务主线、端云分工和责任边界讲清楚，再请研发认领技术接口。”

### 2—8 分钟：只讲产品业务总图

> “产品先提前定义一项长期任务；若本期任务中心提供相应控件，由任务中心接收用户操作，再由负责保存状态的业务返回真实结果。具体支持范围、真实状态由谁保存以及怎样同步，需要专项拍板。基础场景条件稳定成立后，系统先建立本次唯一处理记录，再读取这项任务发布前已经确定的判断方案：本地数据足够、规则明确、要求快或弱网可用的由端侧完成；需要模型、视觉工具和更多上下文的由云端协作；端云方案未确认的任务不得发布。无论走端还是云，判断结果都必须明确、未过期并属于本次场景。接着按任务策略决定是无需每次询问、只提醒，还是必须询问用户。只提醒的任务告知后结束，不进入车控；需要执行的任务只有在静默策略已完成安全会签，或者用户对本次动作明确同意后，才回端侧重新检查。最后由赛力斯执行并读取车辆真实状态，真实状态达到目标才算成功；车辆动作结果展示在卡片还是任务中心、由谁回传，仍需确认。”

### 8—11 分钟：专门讲任务中心到 Trigger

> “现有材料只说明任务中心负责让用户看见并操作任务，但谁真正保存系统预设任务的开关状态、怎样把结果交给触发器，还没有正式结论。我需要晓伟和任务系统确认三件事：用户点开启或关闭后交给谁处理；谁能代表这辆车的真实任务状态；触发器在启动、状态变化和断线恢复后怎样拿到最新结果。任一项不清楚，触发器都不能把任务当成已开启。”

### 11—17 分钟：走端侧天气

> “天气是典型端侧链路。任务真的开启后，端侧持续观察天气、路面、车速和当前驾驶模式；雨天、湿滑或浓雾条件稳定成立后，车机卡片询问用户。用户点击，或者通过可见即可说表达同意后，结果回到天气原业务；端侧再检查一次最新天气和车况，仍满足才切换驾驶模式或打开后雾灯。最后读取车辆真实模式或灯状态，真的变化了才告诉用户成功。”

### 17—24 分钟：走端云充电

> “下面用充电任务演示建议链路，这还不是已经接通的现状。已确认的是云端可以协作调用视觉能力判断附近是否有充电设备，车辆动作最终仍在端侧完成；但谁先发起、使用哪种卡片、点击和语音结果交给谁，还需要专项拍板。建议流程是：车辆满足停车充电的基础条件后，先为本次场景建立唯一记录，再请云端识别充电设备。识别到设备只代表场景成立，不代表用户已经同意。卡片真正展示、用户对本次开盖明确同意后，系统才把请求交给端侧；端侧重新检查 P 挡、车辆停稳、任务仍开启、口盖仍关闭，并确认识别结果和用户同意仍有效。最后只有车辆真实口盖已经打开，才算成功。”
...
> “今天先拍五个业务结论：任务中心能做什么、谁保存真实状态；每条任务走端侧还是云端以及谁唯一发起；是否询问用户；执行前由端侧检查什么；什么真实状态才算成功。技术字段、接口和错误码不在产品会上猜，请对应研发 Owner 认领专项、产物和日期。没有拍板的地方继续保留待确认，不作为开发验收口径。”

## 8.5 本场怎样收敛：先拍五个业务结论，再认领研发专项

### 8.5.1 产品在会上先推动这五个业务结论

| 业务结论 | 你要问的问题 | 会议必须留下什么 |
|---|---|---|
| 1. 用户怎样管理任务 | 任务中心一期到底支持查看、开启、关闭、删除和结果回看中的哪些能力？谁保存真实状态？ | 明确范围、状态 Owner、失败后的页面表现 |
| 2. 每条任务怎样分端云 | 规则在哪一侧判断？如果经过端云两侧，谁是本次唯一发起方？ | 四任务各自端云归属；禁止两侧重复发起 |
| 3. 场景成立后怎样与用户交互 | 是无需每次询问、只提醒，还是必须确认？若出卡，点击和语音回哪里？ | 每条任务唯一交互方式；儿童锁/后视镜未安全会签前不得静默执行 |
| 4. 最终执行前检查什么 | 用户同意或云端识别成功后，端侧必须重新检查哪些任务状态、车况和时效？ | 各任务执行前检查清单和失败原因 |
| 5. 怎样才算成功 | 哪个车辆真实状态表示动作完成？失败或读不到怎样告诉用户？ | 每条任务真实成功标准、失败/未知表现和结果去向 |

这五项是你作为产品必须推动的结论。下面九项是研发把这五个业务决定落成可实现、可联调方案时需要继续确认的内容，产品会上不需要逐个背技术字段。

### 8.5.2 研发专项再展开九项框架结论

<details>
<summary>展开查看九项研发框架结论</summary>
...
### 8.5.3 今天不强行拍技术答案，但必须认领 Owner 和截止时间
...
> “今天产品先拍五件事：用户怎样管理任务、每条任务怎样分端云、场景成立后怎样与用户交互、最终执行前检查什么、哪个真实车辆状态才算成功。技术字段和接口不在现场猜，请对应 Owner 认领专项和日期；会后再把输入、判断、输出、失败与现状证据补回这份 PRD。”
